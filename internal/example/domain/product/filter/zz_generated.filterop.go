...
import (
	...
	product "github.com/innoai-tech/postgres-operator/internal/example/domain/product"
...
type ProductByCode struct {
	// 通过 编码 筛选
	Code *filter.Filter[productv1.ProductCode] `name:"product~code,omitzero" in:"query"`
}

func (f *ProductByCode) OperatorType() sqlpipe.OperatorType {
	return sqlpipe.OperatorFilter
}

func (f *ProductByCode) Next(src sqlpipe.Source[product.Product]) sqlpipe.Source[product.Product] {
	return src.Pipe(sqlpipefilter.AsWhere(product.ProductT.Code, f.Code))
}

type ProductSortByCode struct{}

func (f *ProductSortByCode) Name() string {
	return "product~code"
}

func (f *ProductSortByCode) Label() string {
	return "编码"
}

func (f *ProductSortByCode) Sort(src sqlpipe.Source[product.Product], sortBy func(col sqlbuilder.Column) sqlpipe.SourceOperator[product.Product]) sqlpipe.Source[product.Product] {
	return src.Pipe(sortBy(product.ProductT.Code))
}
