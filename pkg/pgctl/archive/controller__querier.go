...
func (c *Controller) ExportArchiveAsTar(ctx context.Context, code archivev1.ArchiveCode, w io.Writer) error {
	root, err := os.OpenRoot(c.DataDir.PgArchivePath(string(code)))
	if err != nil {
		if os.IsNotExist(err) {
			return &archivev1.ErrArchiveNotFound{}
		}
		return err
	}
...
func (c *Controller) ListArchive(ctx context.Context) (*archivev1.ArchiveList, error) {
	...
	if err := filepath.WalkDir(root, func(path string, d fs.DirEntry, err error) error {
		...
		if d.IsDir() {
			a := &archivev1.Archive{
				Code: archivev1.ArchiveCode(d.Name()),
			}
